...
DB:	while (my $line = <BLAST_DB>){
		...
		if ($query_db eq $prev_q_db){
			...
			$aln_length_db{$query_db} = $hits_db[5];
			$q_length_db{$query_db} = $hits_db[3];
;			$id_db{$query_db} = $hits_db[2];
			$hit_start_db{$query_db} = $hits_db[10];
...
open OUT, ">> $bsr_tab";
print OUT "query\thit\tdb_score\tog_score\tbsr\tseq_length\taln_length\tpct_id\thit_start\n";
close OUT;
...
foreach my $id (keys(%score_og)){ 
	if (exists($score_db{$id})){
		...
		close OUT;
		open OUT, ">> $bsr_tab";
		print OUT "$id\t$hit_id_db{$id}\t$score_db{$id}\t$score_og{$id}\t$bsr\t$q_length_db{$id}\t$aln_length_db{$id}\t$id_db{$id}\t$hit_start_db{$id}\n";
	} 
}
...
open NOHIT, ">> $no_hit";
print NOHIT "query\thit\tdb_score\tseq_length\taln_length\tpct_id\thit_start\n";
close NOHIT;

foreach my $id (keys(%score_db)){
	if (! exists($score_og{$id})){
		open NOHIT, ">> $no_hit";                                                                               # due to the differences in database size (very small for gene of interest, large$
		print NOHIT "$id\t$hit_id_db{$id}\t$score_db{$id}\t$q_length_db{$id}\t$aln_length_db{$id}\t$id_db{$id}\t$hit_start_db{$id}\n";	# these are logged for later inspection, as they may include divergent hits to gene of interest.
		close NOHIT;
	}
}
